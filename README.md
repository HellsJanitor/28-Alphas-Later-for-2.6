...
>12/03/26 mod version 2.6.0.2
 ...
 - Changed starting item from medical bandage to medical first aid kit.

 - Removed parts of OCB MicroSplat to restore vanilla textures to biomes.

 - Added bedroll and player bed healing buff(s).

 - Fixed a bug with cooked canned food healing the player.

 - Fixed a bug with inconsistent mod slots for gear.

 - Fixed some localization errors.

 - Moved/removed some audio files from the custom music folders.

 - Fixed some action skill bugs.

 - Changed initial bag carry capacity to 30 slots, up from 20.
...
> Will require a new save
 ...
 - Item quality is now 1-600, changed from vanilla 1-6.

 - Zombie corpses are now immune to explosion damage

 - Items will degrade on repair, depending on the durability repaired. The more broken the item, the lower the item's quality after repairing.

 - Chickens and rabbits now have 'Press (E) to catch:' as hover text.

 - Perks are consolidated under one menu tab, rather than split into the 5 attribute trees.

 - Passive collectors now have a tool that degrades with use (filter for the dew collector, bee for the apiary, and chicken for the coop),
in addition to 'fuel' (jars for the dew collector, flowers for the apiary, and cornmeal for the chicken coop)

 - Compatibility patch included for Izayo's Weapon Pack.

 - Action skills now have 100 levels, up from 50.
